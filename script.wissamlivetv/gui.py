...
try:
    if not os.path.exists(mashpath):
        ...
        Url  = 'https://github.com/WissamHDTV/Wissam-XBMC-Repo/raw/master/skins-update.zip'
        LocalName = 'skins.zip'
        LocalFile = xbmc.translatePath(os.path.join(Path, LocalName))
        try: urllib.urlretrieve(Url,LocalFile)
        except:xbmc.executebuiltin("XBMC.Notification(TV Guide Wissam,Skin download failed,3000)")
